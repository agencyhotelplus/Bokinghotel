...
function openBook(id){
 ...
 $("room").innerHTML=selected.rooms.map(r=>`<option value="${r.id}">${esc(r.name)} — Rp ${fmt(r.price_per_night*0.8)}/malam (diskon 20%)</option>`).join("");
...
function updateSummary(){
 ...
 const discounted=price?Math.round(price*0.8):null;
 $("nightPrice").innerHTML=discounted?`<span class="old">Rp ${fmt(price)}</span> <b>Rp ${fmt(discounted)}</b>`:"Konfirmasi";
 $("total").textContent=(discounted&&n)?`Rp ${fmt(discounted*n*Number($("roomQty").value||1))}`:"Menunggu harga";
}
...
Harga normal/malam: ${price?"Rp "+fmt(price):"Mohon konfirmasi"}
Diskon: 20%
Harga setelah diskon/malam: ${price?"Rp "+fmt(Math.round(price*0.8)):"Mohon konfirmasi"}
Total setelah diskon: ${total?"Rp "+fmt(Math.round(total*0.8)):"Mohon konfirmasi"}
